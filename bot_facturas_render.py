...
TRIMESTRES = {
    ...
    2: os.getenv('GOOGLE_DRIVE_TRIMESTRE_2', 'ID_TRIMESTRE_2'),
    3: os.getenv('GOOGLE_DRIVE_TRIMESTRE_3', '11v7LWkiBw2SjI_x4WZOV1GAI2XDovKjn'),
    4: os.getenv('GOOGLE_DRIVE_TRIMESTRE_4', 'ID_TRIMESTRE_4'),
}
...
def subir_a_google_drive(nombre_archivo, ruta_archivo, tipo_documento):
    """
    Navega automáticamente: Año → Trimestre → Mes → Tipo (Facturas/Presupuestos)
    ...
    try:
        ...
        # Obtener mes y trimestre actual
        ahora = datetime.now()
        ...
        logger.info(f"Procesando {tipo_documento}: {nombre_mes} (mes {mes}/2026), Trimestre {trimestre}")
        
        # Verificar que el trimestre está configurado
        id_trimestre = TRIMESTRES[trimestre]
        if not id_trimestre or id_trimestre.startswith('ID_TRIMESTRE'):
            raise ValueError(f"Trimestre {trimestre} no está configurado en Render. Crea la carpeta en Drive primero y actualiza GOOGLE_DRIVE_TRIMESTRE_{trimestre}")
        
        # Navegar/crear: Trimestre → Mes → Tipo
        ...
        logger.info(f"Archivo subido a Drive: {file.get('id')} en {nombre_mes}/{nombre_tipo}")
