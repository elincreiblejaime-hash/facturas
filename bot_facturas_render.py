...
def subir_a_google_drive(nombre_archivo, ruta_archivo, tipo_documento):
    ...
    try:
        ...
        nombre_año = str(año)
        nombre_trimestre = TRIMESTRES[trimestre]
        nombre_mes = MESES[mes]
        nombre_tipo = TIPOS_DOCUMENTO_CARPETAS[tipo_documento]
        
        logger.info(f"Procesando {tipo_documento}: {nombre_mes} ({mes}/{año}), {nombre_trimestre}")
...
def main():
    ...
    # Crear event loop EXPLÍCITAMENTE (Python 3.14 es estricto)
    asyncio.set_event_loop_policy(asyncio.DefaultEventLoopPolicy())
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    
    # Iniciar servidor HTTP en thread daemon
